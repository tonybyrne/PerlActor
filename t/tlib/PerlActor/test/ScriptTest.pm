...
sub test_getTraceInfo
{
	...
	$self->assert_str_equals("in test_script at 'Dummy', line 1",
		$self->{script}->getTraceInfo());
}

sub test_placeholders
{
	my $self = shift;
	$self->{script}->setPlaceholderValues('a','b','c');

	my $command = $self->{script}->parseLine('Dummy $0 $1 $2');

	my @commandParams = $command->getParams();

	$self->assert_equals(3, scalar @commandParams);

	$self->assert_str_equals('a', $commandParams[0]);
	$self->assert_str_equals('b', $commandParams[1]);
	$self->assert_str_equals('c', $commandParams[2]);
}
